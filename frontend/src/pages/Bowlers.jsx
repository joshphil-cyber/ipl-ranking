...
const SORT_OPTIONS = [
  ...
  { value: 'economy',     label: 'Economy (asc)' },
  { value: 'bowling_avg', label: 'Bowling Avg (asc)' },
]
...
export default function Bowlers() {
  ...
  const filtered = [...players]
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'economy')     return a.economy - b.economy
      if (sortBy === 'bowling_avg') return a.bowling_avg - b.bowling_avg
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-extrabold text-white">
          🎯 IPL 2025 <span className="text-ipl-gold">Bowlers Rankings</span>
        </h1>
        <p className="text-gray-400 mt-1 text-sm">
          Ranked by composite ML score · Economy + Avg + Wickets inverted model · Click for AI report
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(p => (
            <PlayerCard key={p.player} player={p} type="bowler" rank={p.rank} />
          ))}
        </div>
      )}
